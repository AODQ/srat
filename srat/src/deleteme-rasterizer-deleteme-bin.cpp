...
std::tuple<
	std::span<srat::triangle_position_t>,
	std::span<srat::triangle_depth_t>,
	std::span<srat::triangle_perspective_w_t>
> rasterizer_bin_tile_triangle(u32v2 const tileIdx, size_t const triIdx) {
	...
	return {
		std::get<0>(ptr).subspan(cachedIdx, 3),
		std::get<1>(ptr).subspan(cachedIdx, 3),
		std::get<2>(ptr).subspan(cachedIdx, 3),
	};
}
...
void srat::rasterizer_phase_bin_reset(srat::RasterizerBinningConfig const & config) {
...
void srat::rasterizer_bin_triangles(
	srat::RasterizerPhaseBinTrianglesParams const & params
) {
	ImplRasterizerBin & impl = *sRasterizerBinImpl;
