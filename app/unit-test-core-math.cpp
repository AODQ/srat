...
TEST_CASE("simple f32x8") {
	...
	alignas(32) srat::array<f32, 8> data = { 1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f };
	f32x8 v = f32x8_load(data);
	alignas(32) srat::array<f32, 8> out;
	f32x8_store(v, out);
...
TEST_CASE("f32x8 negation") {
	alignas(32) srat::array<float, 8> data = { 1.f, -2.f, 3.f, -4.f, 5.f, -6.f, 7.f, -8.f };
	f32x8 v = f32x8_load(data);
	f32x8 neg = -v;
	alignas(32) srat::array<float, 8> out;
	f32x8_store(neg, out);
	for (size_t i = 0; i < 8; ++i) {
		CHECK(out[i] == -data[i]);
	}
...
TEST_CASE("f32x8 sqrt") {
	alignas(32) srat::array<float, 8> data = { 1.f, 4.f, 9.f, 16.f, 25.f, 36.f, 49.f, 64.f };
	f32x8 v = f32x8_load(data);
...
TEST_CASE("f32x8 comparison and select") {
	alignas(32) srat::array<float, 8> a_data = { 1.f, 5.f, 3.f, 7.f, 2.f, 6.f, 4.f, 8.f };
...
TEST_CASE("f32x8 fmadd") {
	alignas(32) srat::array<float, 8> a_data = { 1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f };
	f32x8 a = f32x8_load(a_data);
	f32x8 b = f32x8_splat(3.f);
...
TEST_CASE("f32v4x8 cross") {
	alignas(32) srat::array<float, 8> out_x, out_y, out_z;
...
TEST_CASE("f32v4x8 normalize") {
	...
	alignas(32) srat::array<float, 8> len_out;
...
TEST_CASE("i32x8 load/store roundtrip") {
	alignas(32) srat::array<int, 8> in  = { 0, 1, -2, 3, -4, 5, -6, 7 };
	alignas(32) srat::array<int, 8> out = { 0 };
...
TEST_CASE("u32x8 load/store roundtrip") {
	alignas(32) srat::array<u32, 8> in  = {
		0u, 1u, 2u, 3u, 0x80000000u, 0xFFFFFFFFu, 42u, 999u
	};
	alignas(32) srat::array<u32, 8> out = { 0 };
...
TEST_CASE("i32x8 lane0") {
	alignas(32) srat::array<int, 8> in = { 123, 1, 2, 3, 4, 5, 6, 7 };
	i32x8 v = i32x8_load(in);
	CHECK(i32x8_lane0(v) == 123);
}

TEST_CASE("u32x8 lane0") {
	alignas(32) srat::array<u32, 8> in = { 123u, 1u, 2u, 3u, 4u, 5u, 6u, 7u };
	u32x8 v = u32x8_load(in);
	CHECK(u32x8_lane0(v) == 123u);
}

TEST_CASE("i32x8 splat/zero") {
	alignas(32) srat::array<int, 8> out;
...
TEST_CASE("u32x8 splat/zero") {
	alignas(32) srat::array<u32, 8> out;
...
TEST_CASE("u32x8 ballot/any/all uses sign bit") {
	...
	alignas(32) srat::array<u32, 8> lanes = {
		0x00000000u, // 0
		0x80000000u, // 1
		0x7FFFFFFFu, // 0
		0xFFFFFFFFu, // 1
...
TEST_CASE("f32x8 lane()") {
	alignas(32) srat::array<float, 8> data = { 10.f, 20.f, 30.f, 40.f, 50.f, 60.f, 70.f, 80.f };
	f32x8 v = f32x8_load(data);
	for (size_t i = 0; i < 8; ++i) {
...
TEST_CASE("f32m44 rotate_x 90deg") {
	f32m44 m = f32m44_rotate_x(std::numbers::pi_v<f32> * 0.5f);
...
TEST_CASE("f32m44 rotate_y 90deg") {
	f32m44 m = f32m44_rotate_y(std::numbers::pi_v<f32> * 0.5f);
...
TEST_CASE("f32m44 rotate_z 90deg") {
	f32m44 m = f32m44_rotate_z(std::numbers::pi_v<f32> * 0.5f);
...
TEST_CASE("f32m44 perspective w component") {
	...
	f32m44 p = f32m44_perspective(std::numbers::pi_v<f32> * 0.5f, 1.f, zNear, zFar);
