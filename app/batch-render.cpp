...
static f32m44 make_orbit_view(SratModel const & model)
{
	...
	f32v3 const sz     = model.size();
	f32 const maxDim   = f32_max(sz.x, f32_max(sz.y, sz.z));
	// if the model is degenerate, fall back to a safe distance
	f32 const radius   = (maxDim > 1e-5f) ? (maxDim * 1.5f) : 1.f;

	// 45° azimuth, 25° elevation
	static constexpr f32 kAz  = 0.7854f; // pi/4
	static constexpr f32 kEle = 0.4363f; // ~25 deg
	f32v3 const eye = {
		center.x + radius * cosf(kEle) * cosf(kAz),
		center.y + radius * sinf(kEle),
		center.z + radius * cosf(kEle) * sinf(kAz),
	};
